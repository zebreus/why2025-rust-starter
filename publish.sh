...
curl -X POST -H "badgehub-api-token: ${BADGEHUB_PROJECT_API_TOKEN}" -F "file=@./${LOCAL_ELF_NAME}" https://badge.why2025.org/api/v3/projects/${PROJECT_NAME}/draft/files/${REMOTE_ELF_NAME} \
&& echo "Uploaded main.elf to a draft"

curl -X POST -H "badgehub-api-token: ${BADGEHUB_PROJECT_API_TOKEN}" -F "file=@./metadata.json" https://badge.why2025.org/api/v3/projects/${PROJECT_NAME}/draft/files/metadata.json \
